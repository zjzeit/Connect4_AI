...
public class BoardEvaluation
{
     ...
     */
    public BoardEvaluation(int player, Board board, int strategy)
    {
    ...
    /**
     * Evaluates a given board to a double value -1 < value < 1 for player 1.
    ...
    public double evaluate(Board board)
    {
        ...
        if(strategy == 2){
        eval += getThreatCount(board, 1)*.09;
        }
        ...
        for(int row = 0; row < board.getNumRows(); row++)
            for(int col = 0; col < board.getNumCols(); col++)
            {
                if(gameBoard[row][col]==0)
                {
                    ...
                    tempBoard = clone(board, gameBoard);
    ...
    public int getMove(Board board, int playerNum)
    {
        ...
        if(playerNum == 1)
        {
            ...
            {
                double tempVal = alphaBeta(x, -2, 2, 2, 1);
        ...
        {
            double bestValue = 1; // initialize best value to highest value
            for(int x = 0; x < board.getNumCols(); x++)
            {
                    Board clone = clone(board);
                    if(clone.legalMove(x))
            ...
            for(GameTreeNode x : gameTree.getRoot().getChildren()) // find best eval
            {
                double tempVal = alphaBeta(x, -2, 2, 1, 1);
                System.out.print(tempVal+", ");
                if(tempVal < bestValue)
                {
                    bestValue = tempVal;
    ...
    public double alphaBeta(GameTreeNode node, double alpha, double beta, int player, int depth)
    {
        ...
        if(player == 1)
        {
            for(int x = 0; x < node.getBoard().getNumCols(); x++)
            {
                ...
                {
                    clone.addMove(x,1);
                    alpha = Math.max(alpha, alphaBeta(new GameTreeNode(clone,x), alpha, beta, 2, depth+1));
                    if(beta <= alpha)
                        return alpha;
                }
            }
        ...
        else    // player is not me
        {
            for(int x = 0; x < node.getBoard().getNumCols(); x++)
            {
                ...
                {
                    clone.addMove(x,2);
                    beta = Math.min(beta, alphaBeta(new GameTreeNode(clone,x), alpha, beta, 1, depth+1));
                    if(beta <= alpha)
                        return beta;
                }
            }
    ...
    public Board clone(Board board, int[][] actualBoard)
    {
        Board returnBoard = new Board(board.getNumRows(),
                                        board.getNumCols(),
                                        board.getNumInARow(),
                                        board.getLookAhead());
        returnBoard.setBoard(actualBoard);
